...
- **Model fallback chains for subagents.** Agents can now define ordered per-agent fallback models via `fallback_models` in markdown frontmatter (CSV or YAML list). Global/project fallbacks can be set in `subagents.json` via `fallbackModels` (or `fallback_models`). Runtime resolution now attempts models in this order: primary (`model` param or agent `model`) → agent fallbacks → settings fallbacks → parent model, automatically retrying when model/session startup or prompt execution fails.
