...
| `fallback_models`   | —              | Comma-separated or YAML-list fallback models (`provider/modelId`) tried in order if the primary model fails                                                                                            |
...
Runtime tuning values set via `/agents` → Settings (max concurrency, default max turns, grace turns, default join mode) and hand-edited advanced fields (for example `fallbackModels`) persist across pi restarts. Two files, merged on load:
...
**Model fallback precedence:** primary model (`model` param or agent `model`) → agent `fallback_models` → global/project `fallbackModels` from settings JSON → parent model.
